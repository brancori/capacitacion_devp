...
async function loadRealDashboardData(userId) {
    ...
    // 🔥 FIX: Aplanamos el array si es necesario
    const profile = Array.isArray(rawProfile) ? rawProfile[0] : rawProfile;

    // ✅ Ahora profile.role viene correcto
...
async function mainInit() {
    ...
    // 🔥 CONSULTA DE PERFIL (Para redirección y permisos)
    ...
    if (profileRowError) {
        console.error(" Error al leer perfil:", profileRowError);
        return;
    }

    // 🔥 FIX CORREGIDO: Usamos la variable 'profile' consistentemente
    const profile = Array.isArray(rawProfile) ? rawProfile[0] : rawProfile;
    ...
    const myTenant = profile.tenant_id; // ✅ CORREGIDO: usamos 'profile', no 'profileRow'
    const myRole   = profile.role;      // ✅ CORREGIDO: usamos 'profile', no 'profileRow'
    ...
    // Cargar datos del dashboard (SI NO FUE REDIRIGIDO)
    ...
    const { data: assignments, error: coursesError } = await supabase
        ...
        .select(`
            progress, due_date, status, 
            articles (id, title, thumbnail_url, status, instructor_name, duration_text)
        `)
        .eq('user_id', userId);
